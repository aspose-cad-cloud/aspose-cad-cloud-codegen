...
function Get-ApiEndpointTableRow ($method, $methodPath, $docPath) {
    return ("*CadApi* | [**$(ConvertTo-NativeMethodName $method.Value.operationId)**]($($docPath)CadApi.md#$(ConvertTo-NativeMethodName $method.Value.operationId)) | " +
...
function Generate-CadApiDoc() {
    $apiFileContent = $apiFileHeader
    $apiFileContent += Get-AllMethodsDoc
    $apiFileContent | Set-Content -encoding UTF8 "$($docsFolder)\CadApi.md"
}
